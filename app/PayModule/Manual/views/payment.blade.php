...
<div class="popup-body">
    ...
    <div class="popup-content">
        ...
        <p>{!! __('Please send :amount :currency to the address below. The token balance will appear in your account only after transaction gets approved by our team.', ['num' => $num, 'amount' => '<strong class="text-primary">'.to_num($transaction->amount, 'max').'</strong>', 'currency' => '<strong class="text-primary">'.strtoupper($transaction->currency).'</strong>']) !!}</p>
        ...
        <form action="{{ route('payment.manual.update') }}" method="POST" id="payment-confirm" class="validate-modern" autocomplete="off">
            ...
            <p class="text-head"><strong>{{ __("To speed up the verification process, please enter the wallet address which you will send payment from.") }}</strong></p>
            <div class="input-item input-with-label">
                <input id="token-address" type="text" name="payment_address" class="input-bordered" placeholder="{{ __('Insert your payment address').(($ext_field=='yes') ? ' *' : '') }}" required>
            </div>
            <ul class="d-flex flex-wrap align-items-center guttar-30px">
                ...
                <li class="pdt-1x pdb-1x"><a href="javascript:void(0)" class="btn btn-danger close-modal">{{__('Cancel Order')}}</a></li>
            </ul>
            @if($ext_field != 'yes')
        ...
        <div class="note note-info note-plane">
            ...
            <p>{{__('Make payments using an exchange (Kraken, Coinbase). You can also use MyEtherWallet, MetaMask, Trustwallet etc.')}}</p>
        </div>
        <div class="gaps-1x"></div>
