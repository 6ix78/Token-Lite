...
class PaypalPay
{
    ...
    public function paypal_success(Request $request)
    {
            ...
            try {
                ...
                if ($payment->status == 'COMPLETED') {
                    ...
                    if ($_old_status == 'deleted') {
                        ...
                        //TokenController::paypalsuc($tranx->id);
                        
        $Transactiondata = Transaction::where('id',$tranx->id)->where('status','approved')->first();    
                    if(!empty($Transactiondata)){
                    $check = User::find(Auth::id());
                    /*$tc = new TC();
                    $token = $Transactiondata->tokens;
                    $calc_token = $tc->calc_token($token, 'array');
                    ...
                    if(empty($credit)){*/
                            ...
                            $trans = array();
                            $trans['checked_by'] = '{"name":"paypal","id":1}';
        ...
        /*
                    ...
                    $trans['checked_by'] = null;
                    $trans['checked_time'] = null;
                    Transaction::where('id',$tranx->id)->update($trans);
                }
            }*/
        }
