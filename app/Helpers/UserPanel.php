...
class UserPanel
{
    ...
    public static function user_balance_card($data = null, $atttr = '')
    {
        ...
        $base_con = to_num_token($user->tokenBalance*0.5);    
        $base_out = '<li class="token-balance-sub"><span id="isthisUSD" class="lead">' . ($base_con > 0 ? $base_con : '~') . '</span><span class="sub">' . strtoupper($base_cur) . '</span></li>';
        ...
        $contribute = ($base_out || $cur1_out || $cur2_out) ? '<div class="token-balance token-balance-s2"><h6 class="card-sub-title">' . __('Your Balance in') . '</h6><ul class="token-balance-list">' . $base_out . '</ul></div>' : '';

//this is for OSIS Current Balance
        $return = '<div' . $g_id . ' class="token-statistics card card-token' . $g_cls . '">
        <div class="card-innr"><div class="token-balance' . $ver_cls . '">' . $ver_icon . '
        <div class="token-balance-text"><h6 class="card-sub-title">' . __('Current Balance') . '</h6>
        <span id="whatisthis" class="lead">' . to_num_token($user->tokenBalance) . ' <span>' . token('symbol') . ' <em class="fas fa-info-circle fs-11" data-toggle="tooltip" data-placement="right" title="'.__('Equivalent to'). ' ' .token_price($user->tokenBalance, base_currency()).' '.base_currency(true).'"></em></span></span>
        </div>
    ...
    public static function user_token_block($data = '', $atttr = '')
    {
        ...
        if ($vers =='buy') {
            ...
            $return .= '<div class="gaps-0-5x"></div><div class="d-flex align-items-center justify-content-between mb-0"><a href="'.route('user.token').'" class="btn btn-md btn-primary">'.__('Get OSIS').'</a></div>';
    ...
    public static function user_account_status($data = null, $atttr = '')
    {
        ...
        $user = auth()->user();
        $heading = '<h6 class="card-title card-title-sm">' . __('Account Status') . '</h6><div class="gaps-1-5x"></div>';
        ...
        if(!is_kyc_hide()) {
            if (isset($user->kyc_info->status) && $user->kyc_info->status == 'approved') {
                ...
                $kyc_staus = '<li><a href="' . route('user.kyc') . '" class="btn btn-xs btn-auto btn-info"><span>' . __('Verify ID') . '</span></a></li>';
            }
        }
    ...
    public static function user_account_wallet($data = null, $atttr = '')
    {
        ...
        $uwallet = '<h6 class="card-title' . $title_cls . '">' . __('OSIS Wallet Address') . '</h6><div class="gaps-1x"></div>';
        $uwallet .= '<div class="d-flex justify-content-between align-items-center">';
		
        if ($user->walletAddress) {
            $uwallet .= '<input class="position-absolute" id="wallet_address" value="'.$user->walletAddress.'" style="opacity: 0;"><span>' . show_str($user->walletAddress, 8) . '</span> ';
        ...
        $uwallet .= '<button data-toggle="tooltip" data-placement="top" title="Copied!" class="btn copy_btn" data-clipboard-target="#wallet_address"><i class="fad fa-clipboard"></i></button></div> ';
    ...
    public static function user_kyc_info($data = null, $atttr = '')
    {
        ...
        $ukyc = $heading . '<p>' . __('You may purchase OSIS & get your MetaBadge before verifying ID. Verification will be required in order to participate in OSIS public launch this summer.') . '</p>';
        if (!isset($user->kyc_info->status)) {
            $ukyc .= '<p class="lead text-light pdb-0-5x">' . __('You have not submitted your ID for verification.') . '</p><a href="' . route('user.kyc.application') . '" class="btn btn-sm m-2 btn-icon btn-primary">' . __('Get Verified') . '</a>';
        }
    ...
    public static function token_sales_progress($data = null, $atttr = '')
    {
        ...
        $title .= '<div class="card-head"><h5 class="card-title card-title-sm">'. __('OSIS Sale').'</h5></div>';

        $progress .= '<ul class="progress-info"><li><span>'.__('Raised Amount').' <br></span>'.ico_stage_progress('raised', $sales_raised).'</li><li><span>'.__('Available for Sale').' <br></span>'.ico_stage_progress('total', $sales_total).'</li></ul>';
        ...
        $sales_state = '';
        if (!hide_opt_count()) {
            $sales_end_in .= '<span class="card-sub-title ucap mgb-0-5x">'.__('FireSale Ends In').'</span><div class="countdown-clock" data-date="'._date(active_stage()->end_date, 'Y/m/d H:i:s').'"></div>';
    ...
    */
    public static function user_referral_info($data=null, $atttr='')
    {
        ...
        $refers     .= '<p class="pdb-0-5x"><strong>'.__('Invite friends & family with your unique referral link.').'</strong></p>';
