...
$notes = '<div class="note note-plane note-danger"><em class="fas fa-info-circle"></em><p>'.__("DO NOT USE your exchange wallet address OR if you don't have a private key for the address. You WILL NOT receive your OSIS and WILL LOSE YOUR FUNDS if you do.").'</p></div>';
    ...
    <div class="modal-dialog modal-dialog-md modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="popup-body popup-body-md">
                ...
                <p>{!!__('In order to receive your :SYMBOL, please enter your external wallet address. You will receive :SYMBOL to this address after the OSIS sale ends.', ['symbol' => '<strong>'.token_symbol().'</strong>' ]) !!}</p>
                <div class="gaps-1x"></div>
                @if(!$no_wallet)
                    @if(has_wallet())
                    <ul class="nav nav-tabs"  role="tablist">
                        <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#wallet">{{__('Current Wallet')}}</a></li>
                        <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#request">{{__('Request to change')}}</a></li>
                    </ul>
                    @endif
                    ...
                    <div class="tab-pane fade" id="request">
                        <form class="validate-modern _reload" action="{{ route('user.ajax.account.update') }}" method="POST" id="nio-user-wallet-request" autocomplete="off">
                            ...
                            <div class="gaps-3x"></div>
                            <div class="d-sm-flex justify-content-between align-items-center">
                                <button type="submit" class="btn btn-primary">{{__('Request to Update Wallet')}}</button>
                            </div>
                        </form>{{-- form --}}
                    </div>
                    @endif
