...
<div class="content-area card">
    <div class="card-innr">
        ...
        <div class="card-bordered nopd">
                ...
                <div class="row guttar-vr-15px align-items-center">
                    <div class="col-md-8">
                        <div class="total-block">
                            ...
                            <p class="total-note">{{ __('Equivalent to') }} <span>{{ (to_num($TAC->current_in_base, 'max') * 2).' '.$base_symbol }}</span></p>
                        </div>
                    </div>
                    <div class="col-md-4 text-md-right">
                        <a href="{{ route('user.token') }}" class="btn btn-primary">{{ __('Get More OSIS') }}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="card-innr">
                ...
                <div class="total-block">
                    <ul class="list total-wg">
                        ...
                        <li>
                            <span class="total-title-xs">{{ __('Referral Rewards') }}</span>
                            <span class="total-amount-sm">{{ to_num_token($TAC->referral).' '.$symbol }}</span>
                        </li>
                        <li>
                            <span class="total-title-xs">{{ __('Bonus Tokens') }}</span>
            ...
            <div class="card-innr">
                <div class="total-block">
                    <h5 class="total-title-sm">{{ __('Total Balance') }}</h5>
                    <span class="total-amount">{{ (to_num($TAC->contributed, 'max') * 2).' '.$base_symbol }}</span>
                </div>
                ...
                <div class="card-bordered nopd">
                    ...
                    <div class="card-innr">
                        <div class="has-action">
                            <div class="total-block">
                                <h6 class="total-title-xs">{{ __('Total Balance') }}</h6>
                                <span class="total-amount">{{ (to_num($ST->contribute, 'max') *2).' '.$base_symbol }}</span>
                            </div>
                                ...
                                <div class="toggle-class dropdown-content dropdown-content-md dropdown-content-up-left">
                                    <div class="total-block">
                                        ...
                                        <ul class="list guttar-vr-12px">
                                            ...
                                            <li>
                                                <span class="total-amount-sm">{{ to_num($amt, 'max') * 2}}</span>
                                                <span class="total-title-xs">{{ strtoupper($cur) }}</span>
                                            </li>
                                            @endforeach
                                        </ul>
