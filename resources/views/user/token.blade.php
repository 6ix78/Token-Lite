...
$decimal_max = (token('decimal_max')) ? 2 : 0;
...
<div class="content-area card">
    <div class="card-innr">
        <form action="javascript:void(0)" method="post" class="token-purchase">
            <div class="card-head">
                <h4 class="card-title">
                {{ __('OSIS FIRESALE ON NOW', ['symbol' => $symbol]) }}
                </h4>
            </div>
            <div class="card-text">
                <p>{{ __('Every purchase of OSIS is kept as a utility, contributing to the OSIS Tokenization Launchpad going live in Q4 2022. 

250 OSIS = White Lotus Book, NFT, and CoinCare Crypto Debit Card with no monthly fees for life. 
1000+ OSIS = All of the above + Standard MetaBadge NFT, Crypto Card, & ArchAngel Status. 
2000+ OSIS Guarantees a Rare MetaBadge. 

Two 1000+ OSIS Referrals = All the benefits above.

Any purchase over 5000 OSIS guarantees you 5 MetaBadges & Ascended Status, which comprises all perks above, with private Founder Meet & Greet, Strategy Meetings, & more ✅', ['symbol' => $symbol]) }}</p>
            </div>
            ...
            <div class="card-head">
                <h4 class="card-title">{{ __('Buy OSIS - Select Amount') }}</h4>
            </div>
            <div class="card-text">
                <p>{{ __('OSIS is on sale for a limited time. Enter the amount of OSIS you would like to purchase. Get 1000 or more to claim your MetaBadge. ') }}</p>
            </div>
            @php
            ...
            <div class="token-overview-wrap">
                ...
                <div class="note note-plane note-danger note-sm pdt-1x pl-0">
                    <p>{{__('Please note - token balance can take up to 12 hours to update on your dashboard. The exact conversion rate & payment amount will be calculated based on exchange rates at the moment the transaction is confirmed.')}}</p>
                </div>
            </div>
            @endif
            ...
            <div class="pay-buttons">
                <div class="pay-buttons pt-0">
                    <a data-type="offline" href="#payment-modal" onclick="setDataLayer()" class="btn btn-primary btn-between payment-btn disabled token-payment-btn offline_payment">{{__('Make Payment')}}&nbsp;<i class="ti ti-wallet"></i></a>
                </div>
                <script>
                ...
                <div class="pay-notes">
                    <div class="note note-plane note-light note-md font-italic">
                        <em class="fas fa-info-circle"></em>
                        <p>{{__('Tokens will appear in your account after the payment has been approved by our team. Please note that :SYMBOL tokens will be distributed after the token sales end-date. MetaBadges can be minted immediately, please contact the team on Instagram or Discord.', ['symbol' => $symbol]) }}</p>
                    </div>
                </div>
            </div>
