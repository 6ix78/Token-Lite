...
<div class="page-ath-form">
    ...
    <h2 class="page-ath-heading">{{__('Sign up')}} <small>{{__('Create an')}} {{__('Account & Claim Your MetaBadge')}}</small></h2>
    <form class="register-form" method="POST" onsubmit="setDataLayer()" action="{{ route('register') }}" id="register">
        @csrf
        ...
        <div class="input-item">
            <input type="email" onPaste="return false" placeholder="{{__('Repeat your Email')}}" class="input-bordered{{ $errors->has('email') ? ' input-error' : '' }}" name="verify-email" required>
        </div>
        <div class="input-item">
        ...
        @if(( application_installed(true)) && ($check_users > 0))
            ...
            <div class="input-item text-left">
                ...
                <label for="agree">{!! __('I agree to receive emails from OSIS. I also agree to the') . ' ' .get_page_link('terms', ['target'=>'_blank', 'name' => true, 'status' => true]) . ((get_page_link('terms', ['status' => true]) && get_page_link('policy', ['status' => true])) ? ' '.__('and').' ' : '') . get_page_link('policy', ['target'=>'_blank', 'name' => true, 'status' => true]) !!}.</label>
            </div>
            @else
            <div class="input-item text-left">
                <label for="agree">{{__('By registering you agree to the terms and conditions and know that you receive occasional marketing and newsletter emails.')}}</label>
            </div>
            @endif
        @else
    ...
    <script>
        ...
        function setDataLayer(){
            ...
            let fn = form.get('name').split(' ')[0];
            let ln = form.get('name').split(' ')[1];
            window.localStorage.setItem('em', form.get('email'));
            window.localStorage.setItem('fn', fn);
