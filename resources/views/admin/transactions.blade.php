                    ...
                    <div class="page-nav-bar justify-content-between bg-lighter">
                        <div class="page-nav w-100 w-lg-auto">
                            <ul class="nav">
                                ...
                                <li class="nav-item {{ (is_page('transactions.pending_bonuses') ? ' active' : '') }}"><a class="nav-link" href="{{ route('admin.transactions', 'pending_bonuses') }}">Pending Bonuses</a></li>
                                ...
                                <div class="relative d-inline-block">
                                    ...
                                    <div class="toggle-class dropdown-content dropdown-content-top-left">
                                        <ul id="more-menu-{{ $trnx->id }}" class="dropdown-list">
                                            ...
                                            @endif
											@if(($trnx->status == 'pending' || $trnx->status == 'new') && ($trnx->tnx_type == 'referral'))
                                                <li><a href="javascript:void(0)" class="open-adjust_refer" data-toggle="modal" data-target="#exampleModal" data-id="{{ $trnx->id }}">
                                                    <em class="far fa-check-square"></em>Approve</a></li>
                                             
											@endif
                                            @if(($trnx->status == 'pending' || $trnx->status == 'onhold')&& ($trnx->tnx_type != 'referral'))
                                                @if($trnx->payment_method == 'bank' || $trnx->payment_method == 'manual' || $trnx->payment_method == 'card')
                                                <li><a href="javascript:void(0)" id="adjust_token"  data-status="approved" data-id="{{ $trnx->id }}">
                                                    <em class="far fa-check-square"></em>Approve</a></li>
                                                @endif
                                            ...
                                            @if($trnx->status == 'canceled')
                                                ...
                                                <li><a href="javascript:void(0)" id="adjust_token" data-toggle="modal" data-target="#exampleModal" data-id="{{ $trnx->id }}">
                                                    <em class="far fa-check-square"></em>Approve</a></li>
                                                @endif
                                            @endif
                                        </ul>
...
<div class="modal fade approve_modal" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content p-4"> 
      <div class="modal-body position-relative p-0 text-center">
        
          <div class="content_modal">
              <h2 class="popup-title p-0">Are you sure?</h2>
              <p>Another transaction will create for receiver and update balance with requested amount once approved.</p>
          </div>
          <form action="<?php echo action('Admin\TransactionController@approve_refer')?>" method="post">
        {{csrf_field()}}
        <input type="hidden" name="txnid" id="txnid" value="">
        <div class="">
            <input type="submit" class="btn btn-success" name="Approve" value="Yes, Approved it!"> 
...
$(document).on("click", ".open-adjust_refer", function () {
     var myBookId = $(this).data('id');
	 document.getElementById("txnid").value = myBookId;
});
</script>
